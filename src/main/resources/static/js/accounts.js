...
function newAccount() {
	...
	if (acc) {
		...
		$('#parent').combotree('setValue', '0'); // select top level first
		// element
	...
	if (row) {
		$.messager.confirm('Confirm', 'Are you sure you want to remove this record?', function(r) {
			if (r) {
				...
				//										'Error removing account, please ensure to remove all linked transactions first.');
				//								$.messager.show({ // show
				//									// error message
				//									title : 'Error',
				//									msg : result.msg
				...
				jQuery.ajax({
					url : 'rest/accounts/removeAccount.do',
					async : false,
					data : {id: row.id},
					headers : getCsrfHeaders(),
					type : 'POST',
